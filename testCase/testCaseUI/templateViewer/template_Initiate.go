		...
		for _, file := range *importedFilesPtr {
			if file.Name == selected {
				...
				for index, object := range myContainerObjects {
					if object == richText {
						richText, _ = parseAndFormatText(file.FileContentAsString, &testDataPointValues, randomUuidForScriptEngine)
				...
				for index, object := range myContainerObjects {
					if object == richTextWithValues {
						_, richTextWithValues = parseAndFormatText(file.FileContentAsString, &testDataPointValues, randomUuidForScriptEngine)
...
func parseAndFormatText(inputText string, testDataPointValuesPtr *map[string]string, randomUuidForScriptEngine string) (
