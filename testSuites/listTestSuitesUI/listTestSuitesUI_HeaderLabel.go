...
func newSortableHeaderLabel(headerText string, tempIsSortable bool, tempColumnNumber int, listTestSuiteUI *ListTestSuiteUIStruct) *sortableHeaderLabelStruct {
	...
	tempSortableHeaderLabel.sortImage = newClickableSortImage(
		func() {
			...
			if listTestSuiteUI.currentSortColumn != tempSortableHeaderLabel.columnNumber {

				// New column to sort on
				previousSortColumn = listTestSuiteUI.currentSortColumn
				listTestSuiteUI.currentSortColumn = tempSortableHeaderLabel.columnNumber

				if listTestSuiteUI.currentHeader != nil {
					listTestSuiteUI.previousHeader = listTestSuiteUI.currentHeader

					// Reset the previous header's sort order to Unspecified
					listTestSuiteUI.previousHeader.latestSelectedSortOrder = SortingDirectionUnSpecified
					listTestSuiteUI.previousHeader.sortImage.latestSelectedSortOrder = SortingDirectionUnSpecified

					// Refresh the previous header's sort image and widget
					listTestSuiteUI.previousHeader.sortImage.Refresh()
					listTestSuiteUI.previousHeader.Refresh()
				...
				listTestSuiteUI.currentHeader = tempSortableHeaderLabel
			...
			listTestSuiteUI.sort2DStringSlice(listTestSuiteUI.testSuitesListTableTable, listTestSuiteUI.currentSortColumn, tempSortableHeaderLabel.latestSelectedSortOrder)
			listTestSuiteUI.testSuiteListTable.Refresh()
